...
<div class="container">
	...
	<div class="row">
		<form method="POST" action="AdminAvisCtrl.php">
				...
				<div class="input-field col s12">
					<select name="id_membre" class="browser-default">
						...
						<?php foreach ($membres as $membre) : ?>
							<option value="<?= $membre['id'] ?>" <?= $membre['id'] == $avis->id_membre ? 'selected' : '' ?>><?= $membre['email'] ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				<div class="input-field col s12">
					<select name="id_salle" class="browser-default">
						...
						<?php foreach ($salles as $salle) : ?>
							<option value="<?= $salle['id'] ?>" <?= $salle['id'] == $avis->id_salle ? 'selected' : '' ?>><?= $salle['titre'] ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
			<div class="row">
				<div class="input-field col s12">
					<textarea id="commentaire" class="materialize-textarea" name="commentaire"><?= $avis->commentaire ?></textarea>
			...
			<div class="row">
				<p>Note</p>
				<p class="range-field">
      				<input type="range" name="note" min="0" max="10" value="<?= $avis->note ?>" />
    			</p>
			</div>
			<div class="row">
				<div class="col s12">
